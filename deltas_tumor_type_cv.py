    ...
    if rec:
    	for r in rec :
    		...
    		#dbDic[ strip()]
                    ...
                    if r['tumor_type'] not in uniq_tum_in_file and v not in uniq_code_in_file: # and r['tissue_type'] not in uniq_ts_in_file:
                        del_records.append( [r['tissue_type'] , r['tumor_type'] ,v ])
                    elif r['tumor_type'] not in uniq_tum_in_file  and r['tissue_type'] not in uniq_ts_in_file and [r['tissue_type'] , r['tumor_type'] ,v ] not in del_records:
                        del_records.append( [r['tissue_type'] , r['tumor_type'] ,v ])
                    elif r['tumor_type'] not in uniq_tum_in_file and (r['tissue_type'] in uniq_ts_in_file and v in uniq_code_in_file) and [r['tissue_type'] , r['tumor_type'] ,v ] not in del_records:
                        del_records.append( [r['tissue_type'] , r['tumor_type'] ,v ])
...
def parse_csv(f):
    if not os.path.exists(f):
        ...
        for r in rws[1:]:
            ...
            dic_set_list(r[1],uniq_tum_in_file)
            dic_set_list(r[0],uniq_ts_in_file)
            dic_set_list(r[2],uniq_code_in_file)
...
def diff(f):
    ...
    _write_file('deleted_tumor.txt',del_records)
